...
public class ProcessTime : MonoBehaviour
{
    ...
    void Export()
    {
        ...
        stringBuilder.AppendLine("Diagnostic Step,Propagator Time");

        for (int i = 0; i < propagatorMS.Length; i++) { if(propagatorMS[i] != 0){ stringBuilder.AppendLine(i + "," + propagatorMS[i].ToString("G17")); } }
